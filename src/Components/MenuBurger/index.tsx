    ...
    const { i18n } = useTranslation()
        ...
        return (
            <>
                ...
                <motion.div initial={{ x: 100, opacity: 0 }} animate={isActive ? "render" : "hide"} variants={variants} style={{ position: "fixed", zIndex: 999, background: "#f6f6f6", height: "100vh", width: "330px", display: "flex", flexDirection: "column", justifyContent: "space-between", alignItems: "start", left: "30%", padding: "85px 35px 175px 35px" }}>
                    ...
                    <motion.div initial={{ x: 200, opacity: 0 }} variants={variants} custom={5} animate={isActive ? "renderLi" : ""}  style={{ height: "80px", borderRadius: "10px", display:  "flex", right:"30%", top:"0%", flexDirection: "column", gap: "20px"}}>
                        <button style={{width: "55px", borderRadius: "20px"}} onClick={() => handleChangeLanguage("pt-BR")}><Img borderRadius="10px" src={BrazilFlag}/></button>
                        <button style={{width: "55px", borderRadius: "20px"}} onClick={() => handleChangeLanguage("en-US")}><Img borderRadius="10px" src={USFlag}/></button>
                        <button style={{width: "55px", borderRadius: "20px"}} onClick={() => handleChangeLanguage("es-ES")}><Img borderRadius="10px" src={SpainFlag}/></button>
                    </motion.div>
                </motion.div>
            </>
        )
